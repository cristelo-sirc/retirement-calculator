...
function CoverInputs({ mode = 'essentials' }) {
  const [params, setParams] = React.useState(window.MockEngine.DEFAULTS);
    ...
    <window.CoverChrome active="quiz" tag={`Concept 07 / Cover · Questionnaire · ${detailed ? 'Option B — everything shown' : 'Option A — essentials + reveal'}`}>
      <div style={{ maxWidth: 920, margin: '0 auto', padding: '48px 32px 0' }}>
        ...
        <CGroup title="Guaranteed income" mode={mode}
          advanced={[
            <CField key="pen" field="pension" label="Your pension / mo" value={params.pension} step={250} min={0} max={15000} onChange={v => update('pension', v)} format={v => '$' + v.toLocaleString()} theme={theme} />,
            <CField key="oth" field="otherIncome" label="Other income / mo" value={params.otherIncome} step={250} min={0} max={15000} onChange={v => update('otherIncome', v)} format={v => '$' + v.toLocaleString()} theme={theme} />,
            partner && <CField key="spen" field="spousePension" label="Partner's pension / mo" value={params.spousePension} step={250} min={0} max={15000} onChange={v => update('spousePension', v)} format={v => '$' + v.toLocaleString()} theme={theme} />,
          ].filter(Boolean)}>
          <CField field="ssBenefit" label={partner ? 'Your SS / mo (at 67)' : 'SS / mo (at 67)'} value={params.ssBenefit} step={100} min={0} max={5000} onChange={v => update('ssBenefit', v)} format={v => '$' + v.toLocaleString()} theme={theme} />
          ...
          {partner && <CField field="spouseSS" label="Partner's SS / mo" value={params.spouseSS} step={100} min={0} max={5000} onChange={v => update('spouseSS', v)} format={v => '$' + v.toLocaleString()} theme={theme} />}
