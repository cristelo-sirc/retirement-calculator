...
function CoverNav({ active }) {
  ...
  return (
    <nav style={{ display: 'flex', gap: 26, justifyContent: 'center', paddingTop: 14, flexWrap: 'wrap' }}>
      {tabs.map(t => (
        <span key={t.id} style={{ fontFamily: cvStyles.body, fontSize: 10.5, letterSpacing: '0.18em',
...
// ============================================================================
function CoverCharts() {
  const [params] = React.useState(window.MockEngine.DEFAULTS);
  const results = React.useMemo(() => window.MockEngine.compute(params), [params]);
  const fmt = window.MockEngine.formatCurrency;
